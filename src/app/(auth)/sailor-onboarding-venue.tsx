...
import { useSailorOnboardingState } from '@/src/hooks/useSailorOnboardingState';

const screenWidth = Dimensions.get("window").width;

export default function LocationScreen() {
  const router = useRouter();
  const { state, updateVenue } = useSailorOnboardingState();
  ...
  const handleSelectVenue = () => {
    if (detectedLocation.name) {
      updateVenue({
        venueId: detectedLocation.name.toLowerCase().replace(/\s+/g, '-'),
        venueName: detectedLocation.name,
        venueAddress: detectedLocation.address,
        confidence: detectedLocation.confidence,
      });
      router.push('/(auth)/sailor-onboarding-boat');
    }
  };
  ...
  return (
    <View testID="onboarding-sailor" className="flex-1 bg-white">
      {/* Progress Indicator */}
      <View className="px-4 pt-4 bg-white">
        <OnboardingProgress
          currentStep={1}
          totalSteps={4}
          stepLabels={['Venue', 'Boat', 'Crew', 'Review']}
          color="#0066CC"
      ...
      <View className="px-4 py-4 border-t border-gray-200">
        <TouchableOpacity
          ...
          onPress={handleSelectVenue}
          disabled={!detectedLocation.name}
        >
        ...
        <TouchableOpacity
          ...
          onPress={handleSkip}
        >
          <Text className="text-blue-600 font-medium">Skip for now</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
