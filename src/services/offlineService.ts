  ...
  public async cacheNextRace(raceId: string, userId: string): Promise<void> {
    try {
      ...
      const { data: race, error: raceError } = await supabase
        ...
        .select(`
          *,
          venue:sailing_venues!inner(
            *,
            intelligence:regional_intelligence(*),
            cultural:cultural_profiles(*)
          )
        `)
        .eq('id', raceId)
        .single();
      ...
      if (race.venue) {
        await this.cacheVenue(race.venue.id, 'race');
      }
      ...
      if (boat) {
        const { data: tuningGuides } = await supabase
          .from('tuning_guides')
          .select('*')
          .eq('boat_class', boat.boat_class)
          .limit(5);

        if (tuningGuides) {
          await this.setCachedItem(
            `${STORAGE_KEYS.CACHED_TUNING_GUIDES}:${boat.boat_class}`,
            tuningGuides,
            { priority: 'race' }
          );
        }
      ...
      if (race.venue?.latitude && race.venue?.longitude) {
        await this.cacheWeather(race.venue.id, race.venue.latitude, race.venue.longitude);
      }
  ...
  public async cacheUpcomingRaces(userId: string): Promise<void> {
    try {
      ...
      const { data: races, error } = await supabase
        ...
        .lte('race_date', sevenDaysFromNow.toISOString())
