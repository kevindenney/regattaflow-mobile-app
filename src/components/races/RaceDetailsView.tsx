...
export function RaceDetailsView({ raceData, onUpdate }: RaceDetailsViewProps) {
  ...
  // Weather
  const [expectedWindDirection, setExpectedWindDirection] = useState(raceData.expected_wind_direction?.toString() || '');
  const [expectedWindSpeedMin, setExpectedWindSpeedMin] = useState(raceData.expected_wind_speed_min?.toString() || '');
  const [expectedWindSpeedMax, setExpectedWindSpeedMax] = useState(raceData.expected_wind_speed_max?.toString() || '');
  const [expectedConditions, setExpectedConditions] = useState(raceData.expected_conditions || '');
  const [tideAtStart, setTideAtStart] = useState(raceData.tide_at_start || '');
