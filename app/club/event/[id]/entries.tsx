...
import { View, StyleSheet, ScrollView, TouchableOpacity, TextInput, Platform, Alert, ActivityIndicator } from 'react-native';
  ...
  if (Platform.OS === 'ios') {
    ...
    const filteredRegistrations = registrations.filter((reg) => {
        ...
        if (searchQuery) {
            ...
            return (reg.participant_name.toLowerCase().includes(query) ||
    ...
    return (<ThemedView style={styles.container}>
      ...
      <View style={styles.filtersContainer}>
        <View style={styles.searchContainer}>
          ...
          <TextInput style={styles.searchInput} value={searchQuery} onChangeText={setSearchQuery} placeholder="Search by name, boat..." placeholderTextColor="#94A3B8"/>
        </View>

        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.filterTabs}>
          {(['all', 'pending', 'approved', 'waitlist', 'rejected'] as const).map((status) => (<TouchableOpacity key={status} style={[styles.filterTab, filter === status && styles.filterTabActive]} onPress={() => setFilter(status)}>
              <ThemedText style={[
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        {filteredRegistrations.length === 0 ? (<View style={styles.emptyContainer}>
            <Ionicons name="people-outline" size={48} color="#CBD5E1"/>
            <ThemedText style={styles.emptyText}>No registrations found</ThemedText>
          </View>) : (filteredRegistrations.map((registration) => (<View key={registration.id} style={styles.registrationCard}>
              <View style={styles.registrationHeader}>
                ...
                { backgroundColor: `${getStatusColor(registration.status)}20` },
                  ...
                  <ThemedText style={[styles.statusText, { color: getStatusColor(registration.status) }]}>
                    {registration.status}
                  </ThemedText>
                </View>
              </View>

              {registration.boat_name && (<View style={styles.boatInfo}>
              ...
              {registration.crew_count > 0 && (<View style={styles.crewInfo}>
              ...
              {/* Payment Status */}
              {event?.registration_fee && event.registration_fee > 0 && (<View style={styles.paymentInfo}>
                  <View style={[
                    styles.paymentStatusBadge,
                    { backgroundColor: `${getPaymentStatusColor(registration.payment_status)}20` },
                ]}>
                    <Ionicons name={getPaymentStatusIcon(registration.payment_status)} size={16} color={getPaymentStatusColor(registration.payment_status)}/>
                    <ThemedText style={[
                  ...
                  {registration.amount_paid && (<ThemedText style={styles.amountPaid}>
                      ${registration.amount_paid.toFixed(2)}
                      {registration.payment_method && ` (${registration.payment_method})`}
                    </ThemedText>)}
                </View>)}

              {registration.status === 'pending' && (<View style={styles.actions}>
                  <TouchableOpacity style={[styles.actionButton, styles.approveButton]} onPress={() => handleApprove(registration.id)}>
                    <Ionicons name="checkmark" size={20} color="#FFFFFF"/>
                    <ThemedText style={styles.actionButtonText}>Approve</ThemedText>
                  </TouchableOpacity>
                  <TouchableOpacity style={[styles.actionButton, styles.waitlistButton]} onPress={() => handleMoveToWaitlist(registration.id)}>
                    <Ionicons name="time-outline" size={20} color="#FFFFFF"/>
                    <ThemedText style={styles.actionButtonText}>Waitlist</ThemedText>
                  </TouchableOpacity>
                  <TouchableOpacity style={[styles.actionButton, styles.rejectButton]} onPress={() => handleReject(registration.id)}>
              ...
              {event?.registration_fee &&
                ...
                registration.status === 'approved' && (<View style={styles.paymentActions}>
                    <TouchableOpacity style={[styles.actionButton, styles.markPaidButton]} onPress={() => handleMarkAsPaid(registration.id, event.registration_fee!)}>
                      <Ionicons name="cash-outline" size={20} color="#FFFFFF"/>
                      <ThemedText style={styles.actionButtonText}>Mark as Paid</ThemedText>
                    </TouchableOpacity>
                    <TouchableOpacity style={[styles.actionButton, styles.waiveButton]} onPress={() => handleWaivePayment(registration.id)}>
                      <Ionicons name="gift-outline" size={20} color="#FFFFFF"/>
                      <ThemedText style={styles.actionButtonText}>Waive</ThemedText>
                    </TouchableOpacity>
                  </View>)}

              {registration.rejection_reason && (<View style={styles.rejectionReason}>
