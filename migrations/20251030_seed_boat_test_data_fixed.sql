-- Boat Performance Data Seeding Migration (Fixed)
...
BEGIN
  ...
  -- Step 2: Check if boat already exists
  SELECT EXISTS(
  ...
  IF v_boat_exists THEN
    ...
    UPDATE sailor_boats
    SET name = v_boat_class_name || ' ' || v_sail_number,
        updated_at = NOW()
    WHERE sailor_id = v_sailor_id AND sail_number = v_sail_number
  ...
  -- Step 5: Create boat crew members (using INSERT without ON CONFLICT)
  -- First delete any existing crew for this boat
  ...
  -- First delete any existing equipment for this boat
  ...
  -- First delete any existing maintenance records for this boat
