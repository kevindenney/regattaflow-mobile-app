-- Fix: get_class_experts function - cast bigint COUNT results to INT
...
BEGIN
  ...
  race_scores AS (
    ...
    SELECT
      ...
      COUNT(*) FILTER (WHERE rr.position <= 3)::INT AS podium_finishes,
      COUNT(*) FILTER (WHERE rr.position <= 10)::INT AS top_10_finishes,
  ...
  sharing_scores AS (
    ...
    SELECT
      r.created_by AS user_id,
      COUNT(*) FILTER (WHERE r.content_visibility = 'public')::INT AS public_races,
      COUNT(*) FILTER (WHERE r.prep_notes IS NOT NULL)::INT AS races_with_prep,
      COUNT(*) FILTER (WHERE r.post_race_notes IS NOT NULL)::INT AS races_with_analysis,
  ...
  SELECT
    ...
    cs.podiums::INT AS podium_count,
    cs.public_count::INT AS public_race_count,
