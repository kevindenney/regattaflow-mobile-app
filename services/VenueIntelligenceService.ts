...
export class VenueIntelligenceService {
  ...
  async saveVenueInsights(insights: VenueInsights): Promise<{ success: boolean; error?: string }> {
    return withErrorHandling(
      ...
      {
        service: 'VenueIntelligenceService',
        method: 'saveVenueInsights',
        fallback: { success: false, error: 'Failed to save insights' },
        context: { venueId: insights.venueId },
      }
    );
  }
  ...
  async deleteInsights(venueId: string): Promise<{ success: boolean; error?: string }> {
    return withErrorHandling(
      ...
      {
        service: 'VenueIntelligenceService',
        method: 'deleteInsights',
        fallback: { success: false, error: 'Failed to delete insights' },
        context: { venueId },
      }
    );
  }
