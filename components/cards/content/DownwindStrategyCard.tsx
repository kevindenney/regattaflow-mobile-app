/**
 * DownwindStrategyCard - Position 8
 *
...
import { ArrowDown, Navigation, ChevronDown } from 'lucide-react-native';
...
export function DownwindStrategyCard({
  ...
  const template = useMemo(() => {
    return getTemplateForCardType('downwind_strategy', detectedRaceType);
  }, [detectedRaceType]);
  ...
  const Icon = isDistanceRace ? Navigation : ArrowDown;
  const color = isDistanceRace ? IOS_COLORS.purple : IOS_COLORS.blue;

  if (!template) return null;

  // Get first section title for collapsed preview
  const firstSectionTitle = template.sections?.[0]?.title || 'Downwind Strategy';
    ...
    return (
      <View style={styles.container}>
        ...
        <View style={styles.headerRow}>
          <Text style={styles.sectionLabel}>DOWNWIND STRATEGY</Text>
          {isDistanceRace && (
