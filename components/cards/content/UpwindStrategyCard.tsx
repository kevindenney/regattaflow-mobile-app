...
import { ArrowUp, Waves, ChevronDown } from 'lucide-react-native';
...
export function UpwindStrategyCard({
  ...
  const template = useMemo(() => {
    return getTemplateForCardType('upwind_strategy', detectedRaceType);
  }, [detectedRaceType]);
  ...
  const Icon = isDistanceRace ? Waves : ArrowUp;
  const color = isDistanceRace ? IOS_COLORS.purple : IOS_COLORS.orange;

  if (!template) return null;

  // Get first section title for collapsed preview
  const firstSectionTitle = template.sections?.[0]?.title || 'Upwind Strategy';
    ...
    return (
      <View style={styles.container}>
        ...
        <View style={styles.headerRow}>
          <Text style={styles.sectionLabel}>UPWIND STRATEGY</Text>
          {isDistanceRace && (
            <View style={[styles.raceTypeBadge, { backgroundColor: `${color}20` }]}>
              <Waves size={12} color={color} />
              <Text style={[styles.raceTypeBadgeText, { color }]}>Distance</Text>
            </View>
          )}
        </View>
