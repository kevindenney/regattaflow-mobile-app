/**
 * DebriefTile - Apple Weather-inspired debrief progress widget
 *
 * Compact 155x155 pressable tile matching the RaceResultTile pattern.
 * Shows debrief completion progress with three visual states:
...
// iOS System Colors (matching RaceResultTile)
const COLORS = {
  blue: '#007AFF',
  green: '#34C759',
  orange: '#FF9500',
...
export interface DebriefTileProps {
  /** Number of questions answered */
  answeredCount: number;
  /** Total number of debrief questions */
  totalQuestions: number;
  /** Whether the debrief is fully complete */
  isComplete: boolean;
  /** Callback when tile is pressed */
  onPress: () => void;
}

export function DebriefTile({
  answeredCount,
  totalQuestions,
  isComplete,
  onPress,
  ...
  return (
    <AnimatedPressable
      ...
      accessibilityLabel={
        isComplete
          ? `Debrief complete: ${answeredCount} answered`
          : answeredCount > 0
            ? `Debrief in progress: ${answeredCount} of ${totalQuestions}`
            : 'Start debrief'
      }
    >
      {/* Completion badge */}
      ...
      <View style={styles.header}>
        <ClipboardList size={12} color={COLORS.orange} />
      ...
      <View style={styles.body}>
        {isComplete ? (
          <>
            ...
            <Text style={styles.completeText}>{answeredCount} answered</Text>
          </>
        ) : (
          <>
            <View style={styles.countRow}>
              <Text style={styles.countLarge}>{answeredCount}</Text>
              <Text style={styles.countSuffix}>/{totalQuestions}</Text>
            </View>
      ...
      <Text style={styles.hint} numberOfLines={1}>
        {isComplete ? 'Tap to review' : answeredCount > 0 ? 'Continue' : 'Start debrief'}
      </Text>
    </AnimatedPressable>
  );
}
