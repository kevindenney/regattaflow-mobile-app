/**
 * Team Racing Covering Interactive
 * Visualizes defensive covering plays (tight cover, loose cover, pairing,
 * downwind defense, finish protection) with animated boat positions
...
import { DEFENSIVE_PLAYS, COVERING_OVERVIEW, type DefensivePlay, type DefensivePlayType } from './data/teamRacingCoveringData';
import { TopDownSailboatSVG } from './shared';

const TYPE_COLORS: Record<DefensivePlayType, string> = {
  'tight-cover': '#3B82F6',
  'loose-cover': '#60A5FA',
  'pairing': '#8B5CF6',
  'downwind-defense': '#10B981',
  'finish-protection': '#F59E0B',
};
const TYPE_LABELS: Record<DefensivePlayType, string> = {
  'tight-cover': 'Tight Cover',
  'loose-cover': 'Loose Cover',
  'pairing': 'Pairing',
  'downwind-defense': 'Downwind Defense',
  'finish-protection': 'Finish Protection',
};
const FILTERS: { label: string; types: DefensivePlayType[] }[] = [
  { label: 'All', types: [] },
  { label: 'Tight Cover', types: ['tight-cover'] },
  { label: 'Loose Cover', types: ['loose-cover'] },
  { label: 'Pairing', types: ['pairing'] },
  { label: 'Downwind Defense', types: ['downwind-defense'] },
  { label: 'Finish Protection', types: ['finish-protection'] },
];

function comboSum(c: string): number {
  return c.split('-').map(Number).reduce((a, b) => a + b, 0);
}

interface TeamRacingCoveringInteractiveProps {
  onComplete?: () => void;
}

export function TeamRacingCoveringInteractive({ onComplete }: TeamRacingCoveringInteractiveProps) {
  ...
  const filteredPlays = useMemo(() => {
    ...
    return g.types.length === 0 ? DEFENSIVE_PLAYS : DEFENSIVE_PLAYS.filter((p) => g.types.includes(p.type));
  }, [filterIdx]);

  const play: DefensivePlay = filteredPlays[playIndex] ?? filteredPlays[0];
  ...
  useEffect(() => {
    if (viewedPlays.size >= DEFENSIVE_PLAYS.length) onComplete?.();
  }, [viewedPlays, onComplete]);
  ...
  const handlePrevPlay = useCallback(
    () => setPlayIndex((p) => (p > 0 ? p - 1 : filteredPlays.length - 1)),
    [filteredPlays.length],
  );
  const handleNextPlay = useCallback(
    () => setPlayIndex((p) => (p < filteredPlays.length - 1 ? p + 1 : 0)),
    [filteredPlays.length],
  );
    ...
    <ScrollView style={s.container} contentContainerStyle={s.content}>
      ...
      <View style={s.svgWrap}>
        <Svg width="100%" height="100%" viewBox="0 0 400 600" preserveAspectRatio="xMidYMid meet">
          <Defs>
            <Polygon id="cov-wa" points="0,0 5,12 -5,12" fill="#64748B" />
          </Defs>
          ...
          {step.marks.map((m, i) => (
            <G key={`mk${i}`}>
              ...
              <SvgText x={m.x + 14} y={m.y + 4} fontSize="9" fill="#94A3B8" fontWeight="500">
                {m.type === 'windward' ? 'WM' : m.type === 'leeward' ? 'LM' : 'F'}
              </SvgText>
            </G>
          ))}
          {/* Boats */}
          {step.boats.map((b) => {
            ...
            return (
              <G key={b.id}>
                {active && (
                  <Circle cx={b.x} cy={b.y} r={20 * pulseVal} fill="none" stroke={color} strokeWidth="2" opacity={0.4} />
                )}
      ...
      <View style={s.comboRow}>
        ...
        <Ionicons name="arrow-forward" size={20} color={isImproving ? '#10B981' : afterSum === beforeSum ? '#3B82F6' : '#EF4444'} />
        <View style={s.comboBlock}>
          <Text style={s.comboLabel}>After</Text>
          <Text style={[s.comboVal, { color: isImproving ? '#10B981' : afterSum === beforeSum ? '#3B82F6' : '#EF4444' }]}>
            {play.combinationAfter} = {afterSum}
          </Text>
        </View>
      </View>
        ...
        <View style={s.infoPanel}>
          <View style={s.rowBetween}>
            ...
            <View style={[s.diffBadge, { backgroundColor: diffColors[play.difficulty].bg }]}>
              <Text style={[s.diffText, { color: diffColors[play.difficulty].fg }]}>
...
export default TeamRacingCoveringInteractive;
